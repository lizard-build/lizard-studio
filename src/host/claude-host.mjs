...
// Secrets that must never reach the on-disk log (the bridge/spawn tokens gate
// browser control and the child's env). Populated once each is generated, then
// scrubbed out of every log line — host.log lives in the install dir and any
// local reader that learned a token could drive the bridge or grab the env.
const LOG_REDACT = new Set();
function log(...args) {
  try {
    let line = new Date().toISOString() + " " + args.join(" ") + "\n";
...
const browserClients = new Map(); // authenticated socket -> chat session
...
LOG_REDACT.add(BRIDGE_TOKEN); // never let it into host.log (it rides in the mcp-config we log at spawn)
...
// ---- quarantine-free claude spawning (macOS) --------------------------------
// Chrome ships LSFileQuarantineEnabled, and macOS propagates that flag down the
// whole process tree: this host, the claude it spawns, and every file any of
// them writes gets the com.apple.quarantine xattr. The Bun-compiled claude
// binary extracts its native .node modules into $TMPDIR under a fresh random
// name on every launch — a quarantined, unsigned native module makes Gatekeeper
// throw its "could not verify … is free of malware" dialog on EVERY session
// spawn, and "Allow Anyway" never sticks because the filename never repeats
// (anthropics/claude-code#14911). Terminal/desktop launches don't carry the
// flag, which is why the CLI alone never shows the dialog.
//
// The escape: let launchd spawn claude instead of us. Submitted jobs start from
// launchd's clean quarantine context, so nothing claude writes is quarantined
// and Gatekeeper stays quiet. Mechanics: `launchctl submit` runs a tiny shim
// (materialized next to this file from the string below, so install.sh and
// selfUpdate need no extra file); the shim dials back on a localhost socket
// with two connections — "ctl" (JSON lines: receives the {cmd,args,cwd,env}
// spec, reports pid/stderr/exit) and "io" (raw bytes relayed to the child's
// stdin/stdout). The spec travels over the socket, never through disk or argv,
// because the env carries secrets. DetachedClaude mirrors enough of the
// ChildProcess surface for the rest of this file, and degrades to a plain
// direct spawn (dialogs and all) if the launchd path doesn't come up in time —
// a chat must never break over cosmetics.
const SHIM_PATH = join(HERE, "spawn-shim.mjs");
const SHIM_SOURCE = `#!/usr/bin/env node
// Generated by claude-host.mjs — do not edit. See "quarantine-free claude
// spawning" there for why this exists. argv: <port> <tokenFile> <spawnId>
import net from "node:net";
...
function run(ctl, io, spec) {
  ...
  // Host gone — don't leave an orphaned claude running headless.
  ctl.on("close", () => {
    try {
      child.kill("SIGKILL");
    } catch {}
...
function materializeShim() {
  try {
    if (existsSync(SHIM_PATH) && readFileSync(SHIM_PATH, "utf8") === SHIM_SOURCE) return true;
    writeFileSync(SHIM_PATH + ".tmp", SHIM_SOURCE, "utf8");
    renameSync(SHIM_PATH + ".tmp", SHIM_PATH);
    return true;
  } catch (err) {
    log("shim: failed to materialize —", err && err.message);
    return false;
  }
}

const DETACH = process.platform === "darwin" && materializeShim();
const SPAWN_TOKEN = randomBytes(32).toString("hex");
LOG_REDACT.add(SPAWN_TOKEN); // never log it either
// The spawn-shim's shared secret is handed over via a 0600 file, not argv:
// `launchctl submit` args are visible in `ps` to every local user, and this
// token authenticates receipt of the child's full env. The file is owner-only,
// rewritten with a fresh token on every host start.
const SPAWN_TOKEN_FILE = join(HERE, ".spawn-token");
let spawnPort = 0;
const pendingSpawns = new Map(); // spawnId -> DetachedClaude awaiting its shim
let nextSpawnId = 1;

if (DETACH) {
  try {
    writeFileSync(SPAWN_TOKEN_FILE, SPAWN_TOKEN, { mode: 0o600 });
    chmodSync(SPAWN_TOKEN_FILE, 0o600); // enforce owner-only regardless of umask
  } catch (err) {
    log("spawn: failed to write token file —", err && err.message);
  }
  const spawnServer = net.createServer((sock) => {
    sock.setNoDelay(true);
    ...
    const onData = (chunk) => {
      ...
      if (nl < 0) {
        if (buf.length > 4096) sock.destroy(); // handshake is one short line
        return;
      }
      ...
      const facade = hello && hello.token === SPAWN_TOKEN ? pendingSpawns.get(hello.spawnId) : null;
      if (!facade) {
        sock.destroy();
        return;
      }
      facade._attach(hello.chan, sock, buf.subarray(nl + 1));
    };
    sock.on("data", onData);
    sock.on("error", () => {});
  });
  spawnServer.on("error", (err) => log("spawn server error", err && err.message));
  spawnServer.listen(0, "127.0.0.1", () => {
    spawnPort = spawnServer.address().port;
    log("spawn server listening on 127.0.0.1:" + spawnPort);
  });
  // Sweep labels left behind by hosts that died before their launchctl remove.
  execFile("launchctl", ["list"], (err, out) => {
    if (err) return;
    for (const line of String(out || "").split("\n")) {
      const cols = line.trim().split(/\s+/);
      if (cols.length >= 3 && cols[0] === "-" && cols[2].startsWith("com.lizard.spawn.")) {
        execFile("launchctl", ["remove", cols[2]], () => {});
      }
    }
  });
}

class DetachedClaude extends EventEmitter {
  constructor(cmd, args, opts) {
    ...
    this._label = "com.lizard.spawn." + process.pid + "." + this._spawnId;
    this._ctl = null;
    this._io = null;
    this._direct = null;
    ...
    // If the launchd path hasn't produced both channels shortly, degrade to a
    // direct spawn — dialogs are annoying, a dead chat is unacceptable.
    this._fallbackTimer = setTimeout(() => this._degrade("timed out waiting for shim"), 5000);
    execFile(
      "launchctl",
      ["submit", "-l", this._label, "--", NODE, SHIM_PATH, String(spawnPort), SPAWN_TOKEN_FILE, this._spawnId],
      (err) => {
        if (err) this._degrade("launchctl submit failed: " + err.message);
      }
    );
  }

  _attach(chan, sock, rest) {
    if (this._done || this._direct) {
      sock.destroy();
      return;
    }
    if (chan === "ctl" && !this._ctl) {
      ...
      sock.on("close", () => {
        ...
        if (!this._done && !this._direct) this._finish(-1, null);
      });
    } else if (chan === "io" && !this._io) {
      this._io = sock;
      if (rest && rest.length) this.stdout.write(rest);
      sock.on("data", (c) => this.stdout.write(c));
    ...
    if (this._ctl && this._io) {
      ...
      // Both channels up — hand the shim its marching orders.
      try {
  ...
  _ctlMsg(m) {
    if (m.type === "spawned") {
      ...
      this._finish(m.code == null ? null : m.code, m.signal || null);
  ...
  _degrade(reason) {
    if (this._done || this._direct) return;
    log("detached spawn: falling back to direct —", reason);
    clearTimeout(this._fallbackTimer);
    pendingSpawns.delete(this._spawnId);
    for (const s of [this._ctl, this._io]) {
      try {
        s && s.destroy();
      } catch {}
    }
    this._ctl = this._io = null;
    execFile("launchctl", ["remove", this._label], () => {});
    let child;
    try {
      child = spawn(this._spec.cmd, this._spec.args, { cwd: this._spec.cwd, env: this._spec.env, stdio: ["pipe", "pipe", "pipe"] });
    } catch (err) {
      this._fail(err);
      return;
    }
    ...
    child.stdout.on("data", (c) => this.stdout.write(c));
    child.stderr.on("data", (c) => this.stderr.write(c));
    child.on("spawn", () => {
      this.pid = child.pid;
      if (this._pendingSig) {
        try {
          child.kill(this._pendingSig);
        } catch {}
        this._pendingSig = null;
      }
      this.emit("spawn");
    ...
    child.on("exit", (code, signal) => this._finish(code, signal));
  }
  ...
  _cleanup() {
    clearTimeout(this._fallbackTimer);
    pendingSpawns.delete(this._spawnId);
    if (!this._direct) execFile("launchctl", ["remove", this._label], () => {});
    // Give the io socket a beat to drain claude's last stdout bytes, then close
...
const cwdById = new Map(); // id -> last opened cwd (persists across session kills)
...
function startClaude({ id, cwd, model, effort, permissionMode, resume }) {
  ...
  // Record this as a folder the user has genuinely opened — configRead/configWrite
  // will only touch a dir from here, so a crafted message can't redirect a
  // hooks/mcp/CLAUDE.md write into an arbitrary directory (see configCwd).
  openedCwds.add(cwd);
  cwdById.set(id, cwd);
  ...
  log("spawning", CLAUDE, args.join(" "), "cwd=", s.cwd, "id=", id, DETACH ? "(via launchd)" : "(direct)");
  let proc;
  try {
    proc = spawnClaude(args, { cwd: s.cwd, env: CHILD_ENV });
  } catch (err) {
...
function harvestCommands(cwd, model) {
  ...
  let proc;
  try {
    proc = spawnClaude(args, { cwd, env: CHILD_ENV });
    proc.stderr.resume(); // nobody reads it — don't let a chatty stderr buffer up
  } catch (err) {
...
function authLogin(id) {
  ...
  let proc;
  try {
    proc = spawnClaude(["auth", "login", "--claudeai"], { cwd: homedir(), env: CHILD_ENV });
  } catch (err) {
...
function configCwd(id, msg) {
  const s = sessions.get(id);
  // This chat's own project dir — from the live session, else the last one it
  // opened (survives an idle/stopped session, so config editing still works
  // when no claude process is running).
  const own = (s && s.cwd) || cwdById.get(id) || null;
  // Honor an explicit msg.cwd ONLY if it's a directory the user actually opened
  // this run. Otherwise ignore it and fall back to this chat's own dir — so a
  // crafted configWrite can't drop hooks/mcp/CLAUDE.md into an arbitrary
  // directory (e.g. someone's $HOME) that would run code when later opened.
