...
(function () {
  ...
  function playDoneChime() {
    if (backgroundRestoring || !soundOnDone) return;
    try {
  ...
  function onHostMessage(msg) {
    ...
    if (msg.type === "backgroundRestoreStart") {
      ...
      for (const state of backgroundRestoreStates) {
        ...
        const chat = makeChat({ ...old, id: state.id, harness: state.agent,
          cwd: state.spec.cwd || old?.cwd, model: state.spec.model || old?.model, sessionId: state.sessionId || old?.sessionId });
        if (old) {
          if (old.historyRequest) clearTimeout(old.historyRequest.timer);
          old.messagesEl.replaceWith(chat.messagesEl);
        } else {
          order.push(chat.id);
          els.stack.appendChild(chat.messagesEl);
        }
    ...
    switch (msg.type) {
      ...
      case "browser":
        ...
        // The background runtime owns browser tools, including while hidden.
        break;
      case "promptResult":
  ...
  function resetChatSession(chat) {
    ...
    chat.backgroundTurnIds = null;
    chat.backgroundQueueKey = null;
    chat.empty = true;
    chat.turnStatusText = "";
    chat.streamBlocks.clear();
    chat.streamedMsgIds.clear();
