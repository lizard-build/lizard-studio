...
test("the panel replays current activity after reconnect and reports later changes", () => {
  const sent = [], retries = [], intervals = [], events = {}, disconnects = [];
  let active = true;
  const chrome = {
  ...
  vm.runInNewContext(readFileSync(new URL("../../src/panel/panel.js", import.meta.url), "utf8"), {
    ...
    window: { RKChat: { hasActiveChats: () => active }, addEventListener: (name, fn) => { events[name] = fn; } },
  ...
  disconnects[0](); const count = sent.length; active = true; intervals[0](); assert.equal(sent.length, count);
